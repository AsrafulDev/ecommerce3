...
- Header/footer builder: `header_style`/`footer_style` varchar(50) default `classic` · `header_all_category_button` tinyint default 1 · `header_all_category_type` varchar(50) default `mega` · `header_top_bar`/`header_sticky` tinyint default 1 · `footer_columns` int default 4 · `header_components`/`footer_components` json null — new shape is `{"rows":[{"id":"top|main|bottom","columns":[{"widths":{desktop,laptop,tablet,phone},"widgets":[{"id","visibility":{desktop,laptop,tablet,phone}}]}]}]}` (3 fixed zones, ≤6 columns/row, Bootstrap 12-grid spans 1–12). Legacy flat lists (`["topbar",…]`) are upgraded on read by `App\Support\HeaderFooterComponents::normalize()`. See `AGENTS.md`/`plan.md` for the full model.
