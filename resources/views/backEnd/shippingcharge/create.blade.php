...
@section('title','{{ __('Create {{ __('{{ __('Shipping') }} Charge') }}') }}')
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold m-0 text-dark">{{ __('Create {{ __('{{ __('Shipping') }} Charge') }}') }}</h4>
        ...
        <div class="d-flex gap-2">
            ...
            <button type="{{ __('submit') }}" form="shippingCreateForm" class="btn btn-primary fw-bold px-4 shadow-sm">
                <i class="fe-plus me-1"></i> Save {{ __('Configuration') }}
            </button>
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-8 col-md-10">
            ...
            <div class="studio-card p-4">
                <form action="{{route('shippingcharges.store')}}" method={{ __('"{{ __('POST') }}"') }} id="shippingCreateForm" data-parsley-validate="">
                    @csrf
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" 
                                   ...
                                   value="{{ old('name') }}" 
                                   id="name" 
                                   placeholder="{{ __('e.g. Outside Dhaka') }}" 
                                   required>
                            @error('name')
                        ...
                        <div class="col-12">
                            <label class="form-label-custom">{{ __('Initial {{ __('Status') }}') }}</label>
                            <div class="status-toggle-box">
                                <div class="status-text">
                                    <h6>{{ __('Active for {{ __('Customer') }}s') }}</h6>
                                    <small>{{ __('{{ __('Enable') }} this area immediately on checkout') }}</small>
                                </div>
