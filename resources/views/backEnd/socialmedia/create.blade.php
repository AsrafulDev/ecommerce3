...
@section('title','Add {{ __('Social {{ __('Media') }}') }}')
...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-bold m-0 text-dark">{{ __('Add {{ __('New') }} {{ __('Platform') }}') }}</h4>
            <span class="text-muted small">{{ __('Connect a new social media profile') }}</span>
        </div>
        <div class="d-flex gap-2">
            <a href="{{route('socialmedias.index')}}" class="btn btn-light border fw-bold text-secondary px-3 rounded-pill">{{ __('Cancel') }}</a>
            <button type="{{ __('submit') }}" form="socialCreateForm" class="btn btn-primary fw-bold px-4 shadow-sm rounded-pill">
                <i class="mdi mdi-content-save-outline me-1"></i> Save {{ __('Platform') }}
            </button>
        </div>
    </div>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-9 col-md-11">
            ...
            <div class="studio-card p-4 p-md-5">
                <form action="{{route('socialmedias.store')}}" method={{ __('"{{ __('POST') }}"') }} id="socialCreateForm" data-parsley-validate="" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <small class="text-muted">{{ __('Example') }}: <b>{{ __('fab fa-facebook-f') }}</b> or <b>{{ __('fe-facebook') }}</b></small>
                        ...
                        <div class="col-12">
                            <div class="status-toggle-box">
                                <div class="status-text">
                                    ...
                                    <small>{{ __('{{ __('Enable') }} this link to be visible on the website frontend') }}</small>
                                </div>
