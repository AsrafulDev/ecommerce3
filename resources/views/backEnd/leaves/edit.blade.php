...
<div class="container-fluid py-4">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8 col-lg-6">
            ...
            <form action="{{ route('admin.leaves.update', $leave->{{ __('id)') }} }}" method={{ __('"{{ __('POST') }}"') }}>
                @csrf
                @method('PUT')
                ...
                <div class="card card-modern">
                    ...
                    <div class="card-header-modern">
                        <div>
                            <h5 class="mb-1 fw-bold text-dark">{{ __('Edit Leave Request') }}</h5>
                            <p class="text-muted small mb-0">{{ __('Modify leave details for an employee.') }}</p>
                        </div>
                        <a href="{{ route('admin.leaves.index') }}" class="btn btn-light btn-sm rounded-pill px-3">
                            <i data-feather="x" style="width:{{ __('14px') }};"></i>{{ __('Cancel') }}</a>
                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="leave_type" class="form-select form-select-custom @error('leave_type') is-invalid @enderror" required>
                                <option value="" disabled>{{ __('Choose {{ __('Leave Type') }}...') }}</option>
                                <option value="casual" {{ $leave->leave_type == 'casual' ? 'selected' : '' }} class="leave-type-option">{{ __('{{ __('Casual') }} Leave') }}</option>
                                <option value="sick" {{ $leave->leave_type == 'sick' ? 'selected' : '' }} class="leave-type-option">{{ __('{{ __('Sick') }} Leave') }}</option>
                                <option value="annual" {{ $leave->leave_type == 'annual' ? 'selected' : '' }} class="leave-type-option">{{ __('{{ __('Annual') }} Leave') }}</option>
                                <option value="emergency" {{ $leave->leave_type == 'emergency' ? 'selected' : '' }} class="leave-type-option">{{ __('Emergency Leave') }}</option>
                                <option value="maternity" {{ $leave->leave_type == 'maternity' ? 'selected' : '' }} class="leave-type-option">{{ __('Maternity Leave') }}</option>
                                <option value="paternity" {{ $leave->leave_type == 'paternity' ? 'selected' : '' }} class="leave-type-option">{{ __('Paternity Leave') }}</option>
                                <option value="unpaid" {{ $leave->leave_type == 'unpaid' ? 'selected' : '' }} class="leave-type-option">{{ __('{{ __('Unpaid') }} Leave') }}</option>
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="reason" class="form-control form-control-custom" rows="4" placeholder="{{ __('Briefly describe the reason...') }}">{{ old('reason', $leave->reason) }}</textarea>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('admin.leaves.index') }}" class="btn btn-light py-2">{{ __('Discard {{ __('Change') }}s') }}</a>
                        </div>
