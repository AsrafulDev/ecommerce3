...
@section('title','Create {{ __('Review') }}')
...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between py-4">
                <div>
                    <h4 class="page-title mb-1 text-dark fw-bold">{{ __('Create {{ __('New') }} {{ __('Review') }}') }}</h4>
    ...
    <form action="{{ route('reviews.store') }}" method={{ __('"{{ __('POST') }}"') }} enctype="multipart/form-data" data-parsley-validate>
        @csrf
        <div class="row">
            ...
            <div class="col-lg-8">
                ...
                <div class="card mb-4">
                    <div class="card-header">
                        <div class="header-icon"><i class="fe-{{ __('message') }}-square"></i></div>
                        <h5 class="card-title">{{ __('{{ __('Review') }} Information') }}</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-4">
                                    ...
                                    <select class="form-control select2" name="product_id" required>
                                        <option value="">{{ __('{{ __('Select {{ __('Product') }}') }}...') }}</option>
                        ...
                        <div class="form-group mb-0">
                            <label class="form-label">{{ __('{{ __('Review') }} Comment') }} <span class="text-danger">*</span></label>
                            <textarea name="review" class="form-control" rows="5" placeholder="{{ __('Write the customer') }}'s review {{ __('here') }}..." required>{{ old('review') }}</textarea>
                            @error('review')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="form-group mb-4">
                            <label class="form-label">{{ __('Star {{ __('Rating') }}') }} <span class="text-danger">*</span></label>
                            <select class="form-control select2" name="ratting" required>
                                <option value="">{{ __('Choose {{ __('Rating') }}...') }}</option>
                        ...
                        <button type="{{ __('submit') }}" class="btn btn-{{ __('submit') }} w-100 rounded-pill mt-4">
                            <i class="fe-check-circle me-1"></i> Save {{ __('Review') }}
                        </button>
                    </div>
                </div>
