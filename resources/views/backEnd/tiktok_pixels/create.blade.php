...
@section('title','{{ __('Add TikTok Pixel') }}')
...
<style>
    ...
    .form-control { background-color: #fbfcff; border: 1px solid #eef2f7; padding: 12px 15px; border-radius: 8px; font-size: {{ __('14px') }}; color: #2d3436; transition: all 0.3s; }
    ...
    .btn-{{ __('submit') }} { background: linear-gradient(45deg, #010101, #1a1a2e); border: none; color: white; padding: 12px 25px; font-weight: 600; border-radius: 50rem; transition: 0.3s; }
    .btn-{{ __('submit') }}:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.3); }
...
<div class="container-fluid">
    <div class="row mb-3 mt-3">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h4 class="page-title mb-0" style="font-weight: 700; color: #2d3436;">{{ __('Add TikTok Pixel') }}</h4>
                <p class="text-muted font-size-13 mb-0">{{ __('Add your TikTok tracking pixel ID.') }}</p>
            </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5 class="card-title">{{ __('TikTok {{ __('Pixel {{ __('Configuration') }}') }}') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{route('tiktok.pixels.store')}}" method={{ __('"{{ __('POST') }}"') }} data-parsley-validate>
                        @csrf

                        <div class="form-group mb-4">
                            <label for="code" class="form-label">{{ __('TikTok Pixel ID') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-lg @error('code') is-invalid @enderror"
                                   name="code" value="{{ old('code') }}" id="code"
                                   placeholder="{{ __('e.g. C3ABC123DEF456789') }}" required>
                            @error('code')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="help-box mt-2">
                                <small class="text-muted">
                                    <strong>কোথায় পাবেন?</strong> {{ __('TikTok Ads') }} {{ __('Manage') }}r → Assets → Events → Web Events → Pixel ID
                                </small>
                            </div>
                        </div>
