...
<div class="container-fluid py-4">
    ...
    <div class="row justify-content-center">
        <div class="col-md-9 col-lg-7">
            ...
            <form action="{{ route('admin.salary_payments.store') }}" method={{ __('"{{ __('POST') }}"') }}>
                @csrf
                ...
                <div class="card card-modern">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="employee_id" id="employee_id" class="form-control select2 form-select-custom @error('employee_id') is-invalid @enderror" required>
                                ...
                                @foreach($employees as $emp)
                                    <option value="{{ $emp->id }}" {{ (request('employee_id') == $emp->id || old('employee_id') == $emp->{{ __('id)') }} ? 'selected' : '' }}>
                        ...
                        {{-- Auto Fill Section ({{ __('Unpaid') }} {{ __('Salaries') }}) --}}
                        @if(count($unpaid{{ __('Salaries') }}) > 0)
                        <div class="auto-fill-box mb-4">
                            ...
                            <label class="form-label-custom">{{ __('Select a calculated (unpa{{ __('id)') }} salary record') }}:</label>
                            <select name="salary_id" class="form-select form-select-custom bg-white">
                                <option value="">-- {{ __('Select to Auto-fill {{ __('Amount') }}') }} --</option>
                                @foreach($unpaid{{ __('Salaries') }} as $salary)
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-md-6">
                                <label class="form-label-custom">{{ __('Payment {{ __('Date') }}') }} <span class="text-danger">*</span></label>
                        ...
                        {{-- {{ __('Amount') }} --}}
                        <div class="mb-4">
                            <label class="form-label-custom">{{ __('Paying {{ __('Amount') }}') }} <span class="text-danger">*</span></label>
                            <div class="input-group">
                                <span class="input-group-text input-group-text-custom fw-bold">৳</span>
                                <input type="{{ __('number') }}" step="0.01" name="amount" id="amount" class="form-control form-control-custom border-start-0 ps-2 @error('amount') is-invalid @enderror" 
                                       value="{{ old('amount') }}" placeholder="{{ __('0.00') }}" required>
                            </div>
                            @error('amount') <span class="text-danger small">{{ $message }}</span> @enderror
                        </div>

                        {{-- {{ __('Method') }} & Transaction --}}
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label class="form-label-custom">{{ __('Payment {{ __('Method') }}') }}<span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6">
                                <label class="form-label-custom">{{ __('Transaction / {{ __('Check') }} ID') }}</label>
                                <input type="text" name="transaction_id" class="form-control form-control-custom" value="{{ old('transaction_id') }}" placeholder="{{ __('Optional') }}">
                            </div>
                        </div>

                        {{-- Bank Details {{ __('({{ __('Optional') }})') }} --}}
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                <label class="form-label-custom">{{ __('Bank/Provider {{ __('Name') }}') }}</label>
                                <input type="text" name="bank_name" class="form-control form-control-custom" value="{{ old('bank_name') }}" placeholder="{{ __('e.g. City Bank') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label-custom">{{ __('Account Number') }}</label>
                                <input type="text" name="account_{{ __('number') }}" class="form-control form-control-custom" value="{{ old('account_{{ __('number') }}') }}" placeholder="{{ __('Account No.') }}">
                            </div>
                        </div>

                        {{-- {{ __('Note') }}s --}}
                        <div class="mb-4">
                            <label class="form-label-custom">{{ __('{{ __('Note') }}s') }}</label>
                            <textarea name="notes" class="form-control form-control-custom" rows="2" placeholder="{{ __('Any comments...') }}">{{ old('notes') }}</textarea>
                        </div>
                        ...
                        <div class="fund-alert mb-4 d-flex align-items-center">
                            ...
                            <div>
                                <strong>{{ __('Warning') }}:</strong> {{ __('This amount will be deducted from your main fund.') }} <br>
                                {{ __('Current') }} {{ __('Balance') }}: <strong>৳{{ number_format(\App\Helpers\FundHelper::balance(), 2) }}</strong>
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        $('select[name="salary_id"]').on('change', function() {
            ...
            var amount = selectedOption.data('amount'); // {{ __('Use') }} data attribute for cleaner value
            
            if (!amount) {
                // Fallback to regex if data-attribute isn't used
                var salary{{ __('Text') }} = selectedOption.text();
                var match = salary{{ __('Text') }}.match(/৳([\d,]+\.?\d*)/);
                if (match) {
            ...
            if (amount) {
                ...
                $('#amount').css('background-color', '#dcfce7').animate({background{{ __('Color') }}: '#fff'}, 1000);
            }
        });
    });
</script>
