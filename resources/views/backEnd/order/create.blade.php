                ...
                <div class="pos-products-wrapper">
                    <div class="row">
                        @foreach($products as $p)
                            @php
                                $img = optional($p->image)->image ?? 'public/assets/images/placeholder.webp';
                            ...
                            <div class="col-6 mb-2 pos-product-wrapper" data-name="{{ strtolower($p->name) }}">
                                <div class="pos-product-card pos-add-product" data-id="{{ $p->id }}">
                                    ...
                                    @endif
                                    <img src="{{ asset($img) }}" onerror="this.src='{{ asset('public/assets/images/placeholder.webp') }}'" class="pos-product-img" alt="">
...
<script type="text/javascript">
    ...
    function cart_refresh() {
        $.ajax({
            ...
            success: function (res) {
                ...
                // Re-apply the SN mutual-exclusion (dropdown) / duplicate check
                // (manual) after every repaint — cross-row, per product.
                $("#cartTable .sn-select-container").each(function () { syncCartSnSelectOptions($(this).data('product-id')); });
                $("#cartTable .sn-manual-container").each(function () { markDuplicateCartSn($(this).data('product-id')); });
            },
        });
    }
    ...
    $(document).on("click", ".cart_increment", function (e) {
        ...
        if (id) {
            $.ajax({
                ...
                error: function (xhr) {
                    // 🚦 Stock ceiling — qty already at max available (unless
                    //    the product allows negative stock)
                    var msg = xhr.responseJSON && xhr.responseJSON.message;
                    if (msg) { toastr ? toastr.error(msg) : alert(msg); }
                },
            });
    ...
    //    🔧 Cross-row: the same physical unit can't be typed into two DIFFERENT
    //    cart lines of the same product either (e.g. the product was added to
    //    the cart twice, as separate rows) — not just within one row's boxes.
    function markDuplicateCartSn(productId) {
        var inputs = $('.sn-manual-container[data-product-id="' + productId + '"] .cart-sn-input').toArray();
        var counts = {};
    ...
    $(document).on("input", ".sn-manual-container .cart-sn-input", function(){
        markDuplicateCartSn($(this).closest('.sn-manual-container').data('product-id'));
    });
    ...
    $('.sn-manual-container').each(function(){ markDuplicateCartSn($(this).data('product-id')); });
    ...
    //    🔧 Cross-row: also disables it in every OTHER cart row's dropdowns for
    //    the same product (e.g. the product was added to the cart twice, as
    //    separate rows) — a serial can only ever be picked once in the whole cart.
    function syncCartSnSelectOptions(productId) {
        var selects = $('.sn-select-container[data-product-id="' + productId + '"] .cart-sn-select');
    ...
    $(document).on("change", ".cart-sn-select", function(){
        ...
        var productId = container.data('product-id');
        syncCartSnSelectOptions(productId);
        var rowId = container.data('id');
        var sns = [];
    ...
    $('.sn-select-container').each(function(){ syncCartSnSelectOptions($(this).data('product-id')); });
