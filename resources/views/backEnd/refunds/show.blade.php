...
@section('title','{{ __('Refund Details') }}')
    ...
    <div class="row align-items-center mb-4">
        <div class="col">
            ...
            <div class="d-flex align-items-center gap-2 text-muted" style="font-size: 13px;">
                <i class="fe-calendar"></i> {{ __('Requested') }}: {{ $refund->created_at->format('d M, Y h:i A') }}
            </div>
        </div>
        <div class="col-auto">
        ...
        <div class="col-lg-8">
            ...
            <div class="card-modern">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between">
                        <div>
                            <span class="label-text">{{ __('{{ __('Current') }} {{ __('Status') }}') }}</span>
                            @if($refund->status == 'pending')
                                <span class="badge-soft bg-soft-warning"><i class="fe-clock"></i> {{ __('Pending {{ __('Approval') }}') }}</span>
                            @elseif($refund->status == 'approved')
                                <span class="badge-soft bg-soft-info"><i class="fe-check-circle"></i> {{ __('{{ __('Approve') }}d & Waiting Payment') }}</span>
                            @elseif($refund->status == 'rejected')
                                <span class="badge-soft bg-soft-danger"><i class="fe-x-circle"></i> {{ __('Request {{ __('Reject') }}ed') }}</span>
                            @elseif($refund->status == 'processed')
                                <span class="badge-soft bg-soft-success"><i class="fe-check"></i> {{ __('Successfully {{ __('Processed') }}') }}</span>
                            @endif
                        </div>
                        <div class="text-end">
                            <span class="label-text">{{ __('{{ __('Total') }} {{ __('Refund {{ __('Amount') }}') }}') }}</span>
                            ...
                            @if($refund->shipping_charge > 0)
                                <small class="text-muted">(Includes {{ __('Shipping') }}: ৳{{ number_format($refund->shipping_charge, 2) }})</small>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-top">
                        <small class="text-muted"><i class="fe-check-square me-1"></i> {{ __('Processed') }} on: {{ $refund->processed_at->format('d M, Y h:i A') }}</small>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card-modern">
                <div class="card-header-modern">
                    <h5 class="section-title"><i class="fe-{{ __('message') }}-square me-2 text-muted"></i> {{ __('{{ __('Reason') }} for Return') }}</h5>
                </div>
                <div class="card-body">
            ...
            <div class="card-modern">
                <div class="card-header-modern">
                    ...
                    <a href="{{ route('admin.order.invoice', ['invoice_id' => $refund->order->invoice_id]) }}" target="_blank" class="btn btn-sm btn-outline-primary rounded-pill">
                        View {{ __('{{ __('Inv') }}oice') }} <i class="fe-external-link ms-1"></i>
                    </a>
                </div>
                <div class="card-body p-0">
                    <div class="row p-4 border-bottom">
                        <div class="col-md-4">
                            <span class="label-text">{{ __('{{ __('{{ __('Inv') }}oice') }} No') }}</span>
                            <span class="value-text fw-bold">#{{ $refund->order->invoice_id }}</span>
                        </div>
                        <div class="col-md-4">
                            <span class="label-text">{{ __('Order {{ __('Date') }}') }}</span>
                            <span class="value-text">{{ $refund->order->created_at->format('d M, Y') }}</span>
                        </div>
                        <div class="col-md-4">
                            <span class="label-text">{{ __('Grand {{ __('Total') }}') }}</span>
            ...
            <div class="card-modern border-start border-4 border-secondary">
                <div class="card-body">
                    <h6 class="fw-bold text-dark mb-2"><i class="fe-clipboard me-2"></i> {{ __('Admin {{ __('Note') }}') }}</h6>
                    <p class="text-secondary mb-1">{{ $refund->admin_note }}</p>
                    @if($refund->processedBy)
                        <small class="text-muted mt-2 d-block">— {{ __('{{ __('Processed') }} by') }}: <strong>{{ $refund->processedBy->name }}</strong></small>
                    @endif
                </div>
            </div>
            @endif
        ...
        <div class="col-lg-4">
                ...
                <div class="card-body">
                    ...
                    @elseif($refund->status == 'approved')
                        ...
                        <button type="button" class="btn btn-primary btn-action-lg" data-bs-toggle="modal" data-bs-target="#processModal">
                            <i class="fe-credit-card"></i> {{ __('Process Payment') }}
                        </button>
                    @else
                        <button class="btn btn-light btn-action-lg text-muted" disabled>
                            <i class="fe-lock"></i> No {{ __('Actions') }} Available
                        </button>
                    @endif
                </div>
            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <span class="value-text text-capitalize">
                                @if($refund->refund_method == 'original_payment') {{ __('Original') }} Payment
            ...
            <div class="card-modern">
                <div class="card-body text-center pt-4">
                    ...
                    <h5 class="text-dark fw-bold mb-1">{{ $refund->customer->name ?? 'Guest {{ __('{{ __('Use') }}r') }}' }}</h5>
                    <p class="text-muted mb-4">{{ $refund->customer->{{ __('phone') }} ?? 'No {{ __('Phone') }}' }}</p>
                    
                    <div class="text-start border-top pt-3">
                        <div class="mb-2">
                            <i class="fe-mail me-2 text-muted"></i> {{ $refund->customer->email ?? 'No {{ __('Email') }}' }}
                        </div>
                        <div class="mb-2">
                            <i class="fe-map-pin me-2 text-muted"></i> 
                            {{ $refund->customer->address ?? '{{ __('N/A') }}' }}, {{ $refund->customer->district ?? '' }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="modal-title fw-bold">{{ __('{{ __('{{ __('Approve') }} Refund') }} Request') }}</h5>
            ...
            <form action="{{ route('admin.refunds.approve', $refund->{{ __('id)') }} }}" method={{ __('"{{ __('POST') }}"') }}>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label class="label-text">{{ __('Admin {{ __('{{ __('Note') }} {{ __('({{ __('Optional') }})') }}') }}') }}</label>
                        <textarea name="admin_note" class="form-control form-control-modern" rows="3" placeholder="{{ __('Add an internal note...') }}">{{ $refund->admin_note }}</textarea>
                    </div>
                </div>
...
<div class="modal fade" id="rejectModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header border-bottom-0 pb-0">
                <h5 class="modal-title fw-bold text-danger">{{ __('{{ __('Reject') }} Request') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('admin.refunds.reject', $refund->{{ __('id)') }} }}" method={{ __('"{{ __('POST') }}"') }}>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label class="label-text">{{ __('{{ __('Reject') }}ion {{ __('Reason') }}') }} <span class="text-danger">*</span></label>
                        <textarea name="admin_note" class="form-control form-control-modern" rows="3" placeholder="{{ __('Why are you rejecting this?') }}" required>{{ $refund->admin_note }}</textarea>
                    </div>
                </div>
                <div class="modal-footer border-top-0 pt-0">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
            ...
            <form action="{{ route('admin.refunds.process', $refund->{{ __('id)') }} }}" method={{ __('"{{ __('POST') }}"') }}>
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="text" name="transaction_id" class="form-control form-control-modern" required placeholder="{{ __('TRX-12345678') }}" value="{{ $refund->transaction_id }}">
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="label-text">{{ __('{{ __('Amount') }}') }}</label>
                ...
                <div class="modal-footer border-top-0 pt-0">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
                    <button type="{{ __('submit') }}" class="btn btn-primary px-4">{{ __('{{ __('Complete') }} Payment') }}</button>
                </div>
            </form>
        </div>
    </div>
