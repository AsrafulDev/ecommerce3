...
@section('title','{{ __('Manage') }} {{ __('IP Block') }}')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-flex align-items-center justify-content-between py-4">
                <div>
                    <h4 class="page-title mb-1 text-dark fw-bold">{{ __('{{ __('IP Block') }} {{ __('Manage') }}r') }}</h4>
        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5 class="card-title">{{ __('Block {{ __('New') }} IP') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{route('customers.ipblock.store')}}" method={{ __('"{{ __('POST') }}"') }} data-parsley-validate>
                        @csrf
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control @error('ip_no') is-invalid @enderror" 
                                   ...
                                   placeholder="{{ __('e.g. 192.168.0.1') }}" required>
                            @error('ip_no')
                        ...
                        <div class="form-group mb-4">
                            ...
                            <textarea class="form-control @error('reason') is-invalid @enderror" 
                                      ...
                                      placeholder="{{ __('Why is this IP being blocked?') }}" required>{{ old('reason', $prefill{{ __('Reason') }} ?? '') }}</textarea>
                            @error('reason')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>

                        <button type="{{ __('submit') }}" class="btn-{{ __('submit') }}">
                            <i class="fe-lock me-1"></i> Block {{ __('IP Address') }}
                        </button>
                    </form>
                </div>
            </div>
                ...
                <div class="card-body">
                    <div class="d-flex align-items-start">
                        ...
                        <p class="mb-0 font-size-13 text-warning">
                            {{ __('Blocked') }} IPs will not be able to access the website. Please be careful.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5 class="card-title">{{ __('{{ __('Blocked') }} IP List') }}</h5>
                </div>
                <div class="card-body">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-dark fw-bold">{{ __('Edit {{ __('IP Block') }}') }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="{{ __('Close') }}"></button>
            </div>
            <div class="modal-body p-4">
                <form action="{{route('customers.ipblock.update')}}" method={{ __('"{{ __('POST') }}"') }} data-parsley-validate>
                    @csrf
                    ...
                    <div class="d-grid">
                        <button type="{{ __('submit') }}" class="btn btn-primary rounded-pill">
                            Update {{ __('Change') }}s
                        </button>
                    </div>
                </form>
