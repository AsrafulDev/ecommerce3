<div class="section-row sortable-item" data-ls-id="{{ $ls->id }}" data-section-id="{{ $ls->section_id }}" data-section-slug="{{ $ls->section->slug ?? '' }}">
    <div class="section-row-header">
        ...
        <span class="badge {{ $ls->is_visible ? 'bg-success' : 'bg-secondary' }} rounded-pill visibility-badge" 
            ...
            {{ $ls->is_visible ? '{{ __("Visible' : '") }}Hidden' }}
        </span>
    ...
    <div class="section-row-body">
        ...
        <div class="settings-panel" id="settings-{{ $ls->id }}">
            <form action="{{ route('layouts.sections.update-settings') }}" method={{ __('"{{ __('POST') }}"') }}>
                @csrf
                ...
                <div class="row g-2">
                    <div class="col-md-4">
                        ...
                        <select name="columns_config" class="form-select form-select-sm">
                            <option value="col-sm-12" {{ $ls->columns_config == 'col-sm-12' ? 'selected' : '' }}>{{ __('{{ __('Full {{ __('Width') }}') }} (1 col)') }}</option>
                    ...
                    <div class="col-md-8">
                        <label class="form-label-pro" style="font-size:10px;">{{ __('Responsive {{ __('Visibility') }}') }}</label>
                    ...
                    <div class="col-12 mt-2">
                        <button type="{{ __('submit') }}" class="btn btn-sm btn-dark rounded-pill px-4">
                            <i class="fe-check me-1"></i> {{ __('Save Settings') }}
                        </button>
                    </div>
                </div>
            </form>
