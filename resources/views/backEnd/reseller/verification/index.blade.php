...
@section('title', 'Reseller {{ __('Verification') }}s')
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold text-dark">
                <i data-feather="shield" class="text-primary me-2"></i> {{ __('Verification') }} Requests
            </h4>
            <p class="text-muted small mb-0">{{ __('{{ __('Review') }} and verify reseller KYC documents.') }}</p>
        </div>
    </div>
    ...
    <div class="card card-modern">
        ...
        <div class="{{ __('filter') }}-box">
            <div class="row g-3">
                ...
                {{-- {{ __('Status') }} Filter --}}
                <div class="col-md-4">
                    <form method="{{ __('GET') }}" action="{{ route('admin.reseller.verification.index') }}">
                        <div class="input-group">
                            <select name="status" class="form-select form-select-modern">
                                <option value="">{{ __('Filter by {{ __('Status') }}') }}</option>
                                <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>{{ __('Pending Requests') }}</option>
                                <option value="approved" {{ request('status') == 'approved' ? 'selected' : '' }}>{{ __('{{ __('Approve') }}d Resellers') }}</option>
                                <option value="rejected" {{ request('status') == 'rejected' ? 'selected' : '' }}>{{ __('{{ __('Reject') }}ed Resellers') }}</option>
                            </select>
                            <button type="{{ __('submit') }}" class="btn btn-dark"><i data-feather="{{ __('filter') }}" style="width: {{ __('14px') }};"></i></button>
                        </div>
                    </form>
                </div>
                    ...
                    <form method="{{ __('GET') }}" action="{{ route('admin.reseller.verification.index') }}">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="keyword" class="form-control form-control-modern border-start-0" 
                                   placeholder="{{ __('Search name, shop, email...') }}" value="{{ request('keyword') }}">
                            <button type="{{ __('submit') }}" class="btn btn-primary fw-bold">{{ __('Search') }}</button>
        ...
        <div class="table-responsive">
            <table class="table table-modern mb-0">
                <thead>
                    <tr>
                        <th width="5%">#</th>
                        <th width="25%">{{ __('Reseller & {{ __('Shop') }}') }}</th>
                        <th width="20%">{{ __('{{ __('Contact') }} Info') }}</th>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="bg-light text-primary fw-bold rounded-circle d-flex align-items-center justify-content-center me-3 border" 
                                         style="width: 40px; height: 40px; font-size: {{ __('14px') }};">
                                        {{ substr($reseller->name, 0, 1) }}
                                    </div>
                                    <div>
                                        <div class="fw-bold text-dark">{{ $reseller->name }}</div>
                                        <div class="small text-muted">{{ $reseller->shop_name ?? '{{ __('N/A') }}' }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="d-flex flex-column small">
                                    ...
                                    {{-- If {{ __('phone') }} exists, add {{ __('here') }} --}}
                                </div>
                            </td>

                            {{-- {{ __('Documents') }} {{ __('Status') }} --}}
                            <td>
                            ...
                            {{-- {{ __('Actions') }} --}}
                            <td class="text-end">
                                <a href="{{ route('admin.reseller.verification.show', $reseller->{{ __('id)') }} }}" class="btn-icon" title="View Details">
        ...
        <div class="p-4 border-top d-flex justify-content-between align-items-center bg-white rounded-bottom">
            <small class="text-muted">{{ __('Showing') }}<strong>{{ $resellers->first{{ __('Item') }}() }}</strong>{{ __('to') }}<strong>{{ $resellers->last{{ __('Item') }}() }}</strong>{{ __('of') }}<strong>{{ $resellers->{{ __('total') }}() }}</strong> requests
            </small>
            <div>
