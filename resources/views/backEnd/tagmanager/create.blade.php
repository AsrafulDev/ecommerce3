...
@section('title','Create {{ __('Tag {{ __('Manage') }}r') }}')
...
<div class="container-fluid">
    ...
    <div class="row mb-3 mt-3">
        <div class="col-12 d-flex justify-content-between align-items-center">
            <div>
                <h4 class="page-title mb-0" style="font-weight: 700; color: #2d3436;">{{ __('Add {{ __('Tag {{ __('Manage') }}r') }}') }}</h4>
                <p class="text-muted font-size-13 mb-0">{{ __('Configure {{ __('Google {{ __('Tag {{ __('Manage') }}r') }}') }} container.') }}</p>
            </div>
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5 class="card-title">{{ __('GTM {{ __('Configuration') }}') }}</h5>
                </div>
                <div class="card-body">
                    <form action="{{route('tagmanagers.store')}}" method={{ __('"{{ __('POST') }}"') }} data-parsley-validate>
                        @csrf
                        
                        <div class="form-group mb-4">
                            <label for="code" class="form-label">{{ __('{{ __('Tag {{ __('Manage') }}r') }} ID') }} <span class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-lg @error('code') is-invalid @enderror" 
                                   name="code" value="{{ old('code') }}" id="code" 
                                   placeholder="{{ __('e.g. GTM-XXXXXXX') }}" required>
                            
                            <small class="text-muted d-block mt-2">
                                <i class="fe-info"></i> Enter your {{ __('Google {{ __('Tag {{ __('Manage') }}r') }}') }} Container ID.
                            </small>
                        ...
                        <div class="form-group mb-4">
                            <div class="d-flex justify-content-between align-items-center border p-3 rounded bg-light">
                                <div>
                                    ...
                                    <small class="d-block text-muted">{{ __('Enable') }} GTM on your site?</small>
                                </div>
                                <label class="switch">
